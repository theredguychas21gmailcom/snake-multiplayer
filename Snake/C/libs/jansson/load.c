    ...
    if(lex)
    {
        ...
        if(saved_text && saved_text[0])
        {
            if(lex->saved_text.length <= 20) {
                ...
                snprintf(msg_with_context, sizeof(msg_with_context), "%s near '%s'", msg_text, saved_text);
        ...
        else
        {
            ...
            if(lex->stream.state == STREAM_STATE_ERROR) {
                ...
                snprintf(msg_with_context, sizeof(msg_with_context), "%s near end of file", msg_text);
                msg_with_context[sizeof(msg_with_context) - 1] = '\0';
                result = msg_with_context;
            }
        }
...
static json_t *parse_json(lex_t *lex, size_t flags, json_error_t *error)
{
    ...
    if(!(flags & JSON_DISABLE_EOF_CHECK)) {
        ...
        if(lex->token != TOKEN_EOF) {
            error_set(error, lex, json_error_end_of_input_expected, "end of file expected (%i)", lex->token);
            json_decref(result);
            return NULL;
        }
    }
